...
var normalizeObj = require('../../app/index.js');
...
describe('NormalizeObj', function () {
	...
	it('nesting single rename', function () {
		...
		expect(normalized).to.have.property('telefone').to.have.property('mobile').to.be.equal(object.telefone.celular);
		expect(normalized).to.have.property('telefone').to.not.have.property('celular');
	});

	it('nesting single rename copy', function () {
		var normalized = normalizeObj(object).copy('telefone.celular', 'telefone.celular2');

		expect(normalized).to.have.property('telefone').to.have.property('celular').to.be.equal(object.telefone.celular);
		expect(normalized).to.have.property('telefone').to.have.property('celular2').to.be.equal(object.telefone.celular);
	});

	it('nesting single change structure', function () {
		var normalized = normalizeObj(object).change('telefone.celular', 'mobile');

		expect(normalized).to.have.property('mobile').to.be.equal(object.telefone.celular);
		expect(normalized).to.have.property('telefone').to.not.have.property('celular');
	});

	it('multiple renames', function () {
		var normalized = normalizeObj(object).change('nome', 'fullname').change('endereco', 'address');
	...
	it('nesting multiple renames', function () {
		var normalized = normalizeObj(object).change('telefone', 'phones').change('phones.trabalho', 'phones.work').change('phones.celular', 'phones.mobile');

		expect(normalized).to.have.property('phones').to.have.property('work').to.be.equal(object.telefone.trabalho);
		expect(normalized).to.have.property('phones').to.have.property('mobile').to.be.equal(object.telefone.celular);
		expect(normalized.phones).to.not.have.property('trabalho');
		expect(normalized.phones).to.not.have.property('celular');
	});

	it('nesting multiple change structure, delete empty parent key', function () {
		var normalized = normalizeObj(object).change('telefone.trabalho', 'work').change('telefone.celular', 'mobile');

		expect(normalized).to.have.property('work').to.be.equal(object.telefone.trabalho);
		expect(normalized).to.have.property('mobile').to.be.equal(object.telefone.celular);
		expect(normalized).to.not.have.property('telefone');
	});
	...
	it('nesting multiple change structure, keep filled parent key', function () {
		...
		expect(normalized).to.have.property('work').to.be.equal(object.telefone.trabalho);
		expect(normalized).to.have.property('telefone');
	});
});
